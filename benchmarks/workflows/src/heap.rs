...
fn allocated(old_size: usize, new_size: usize) {
    let live = if new_size >= old_size {
        LIVE.fetch_add((new_size - old_size) as u64, Relaxed) + (new_size - old_size) as u64
    } else {
        LIVE.fetch_sub((old_size - new_size) as u64, Relaxed) - (old_size - new_size) as u64
    };
...
pub fn begin_at(base: u64) {
    assert!(!ACTIVE.load(Relaxed));
    assert!(LIVE.load(Relaxed) >= base);
...
pub fn self_test() {
    ...
    unsafe {
        ...
        assert_eq!(remaining(base), 0);
        let base = begin();
        let retained = black_box(alloc(Layout::from_size_align(96, 8).unwrap()));
        assert!(!retained.is_null());
        end(base);
        begin_at(base);
        let extra = black_box(alloc(Layout::from_size_align(16, 8).unwrap()));
        assert!(!extra.is_null());
        dealloc(extra, Layout::from_size_align(16, 8).unwrap());
        assert_eq!(
            end(base),
            Stats {
                peak_live_bytes: 112,
                live_after_render: 96,
                requested_bytes: 16,
                allocation_calls: 1,
            }
        );
        dealloc(retained, Layout::from_size_align(96, 8).unwrap());
        assert_eq!(remaining(base), 0);
