...
Compares **ferromark**, **pulldown-cmark 0.13.4**, **Bun `bun_md`**, and
...
The published table rows use three shared fixtures:

- `tables/tables-plain`: 80 tables with plain cell text.
- `tables/tables-commonmark-inline`: the same layout mixing plain text, emphasis,
  and strong emphasis.
- `tables/tables-links`: plain and emphasized labels with a Markdown link column.
...
homepage data from the same raw samples. Pulldown-cmark follows Ferromark in
every displayed comparison. Winners are selected from unrounded measured times;
all exact ties are bold. It rejects short screening protocols,
