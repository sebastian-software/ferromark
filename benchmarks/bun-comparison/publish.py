...
from run import HEADLINE_CASES, PARSERS, mismatches
...
def load_publication(folder):
    ...
    for case, identifier, label, kind in CASES:
        if case not in HEADLINE_CASES or verified[case]["mismatches"] or set(verified[case]["output_sha256"]) != PARSERS:
        ...
        if mismatches(original["outputs"]) or hashes != verified[case]["output_sha256"]:
        ...
        tables.append({"id":identifier, "case":case, "label":label, "kind":kind,
            ...
            "inputBytes":verified[case]["bytes"], "rows":rows})
    return metadata, tables, verified


def data_for(folder):
    metadata, tables, verified = load_publication(folder)
    ...
    return {
        "note":"Generated by benchmarks/bun-comparison/publish.py from verified raw samples. Do not hand-edit figures.",
        "source":source, "ferromarkRevision":metadata["ferromark_revision"],
        ...
        "summary":"Every displayed case passed the five-parser output-equivalence gate and three alternating-order measurement runs. Speed ratios apply to these inputs, feature sets, and allocation lifecycle.",
        "headline":{"table":"commonmark-5k", "parser":"ferromark"},
        "verification":{"eligible":sum(not c["mismatches"] for c in verified.values()), "total":len(verified)},
...
ratios; the report preserves those differences. {data['verification']['eligible']} of
{data['verification']['total']} input/configuration pairs passed the shared output gate.
