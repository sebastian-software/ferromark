...
| `line_comments` | Block/line hot path: checks physical line prefixes while parsing. Comment-bearing paragraphs, definitions, or stripped sub-sources can allocate joined text and a source map. | Changes block, table, definition, and nested-source handling; code and raw HTML remain opaque. See [line-comment handling](../crates/ferromark_parser/src/parser/line_comments.rs#L15). |
...
| `definition_lists` | Option check occurs for each block candidate. When enabled, term/body collectors run multiple block recognizers and matching bodies allocate a dedented source/map and recursively parse. | Definition-list bodies can combine with lists, tables, comments, and other block syntax. See [definition-list collector](../crates/ferromark_parser/src/parser/definition_list.rs#L15). |
