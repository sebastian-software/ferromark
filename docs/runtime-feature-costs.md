...
| `line_comments` | Checks physical line prefixes while parsing. Paragraphs reuse the first comment position observed by block parsing; absent comments need no second discovery scan. Reference definitions retain a marker preflight. Actual removed comments can require joined text and a source map. | Changes block, table, definition, and nested-source handling; code and raw HTML remain opaque. See [line-comment handling](../crates/ferromark_parser/src/parser/line_comments.rs). |
