...
def main():
    ...
    reference_path = ROOT / 'docs/reports/2026-09-14-native-matched/verification.json.gz'
    reference = native.read_json(reference_path)
    ...
    for label, variant in variants.items():
        build_path = Path(variant['build']) / 'build.json'
        build = json.loads(build_path.read_text())
        assert native.sha(build['binary']) == build['binary_sha256']
        if build.get('source') and build['source'] not in checked_sources:
            for filename, expected in build['source_files_sha256'].items():
                assert native.sha(Path(build['source']) / filename) == expected, (label, filename, 'source drift')
            checked_sources.add(build['source'])
        variant['metadata'] = build
        variant['metadata_sha256'] = native.sha(build_path)
    ...
    config = {'rounds': args.rounds, 'pairs': args.pairs, 'window_ms': args.window_ms,
              'warmup_ms': 10, 'all_cases': args.all_cases,
              'corpus_source': str(args.corpus), 'corpus_source_sha256': native.sha(args.corpus),
              'reference_verification': str(reference_path), 'reference_verification_sha256': native.sha(reference_path),
        ...
        for case, mode in jobs:
            ...
            try:
                for label, w in workers.items():
                    ...
                    w.bench(config['warmup_ms'] * 1_000_000, expected[label][mode]['metric'])
