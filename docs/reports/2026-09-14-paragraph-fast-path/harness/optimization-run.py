#!/usr/bin/env python3
"""Verify and measure two frozen workers over a compatible Markdown corpus."""
...
class Worker:
    def __init__(self, binary, profile, mode, paths):
        self.process = subprocess.Popen(
            [str(binary), profile, mode, *map(str, paths)], text=True,
    ...
    def bench(self, budget_ns):
        ...
        return {"iterations": int(iterations), "elapsed_ns": int(elapsed_ns), "checksum": int(checksum)}
...
def host():
    ...
    return {
        ...
        "power": probe(["pmset", "-g", "batt"]), "thermal": probe(["pmset", "-g", "therm"]),
    }


def expected_result(actual, expected, label):
    if len(actual) != 1:
...
def verify_case(build, input_path, case, modes):
    ...
    for engine in ENGINES:
        ...
        for mode in modes[1:]:
            ...
            if (other["html"], other["ast_debug"], other["children"]) != (
                first["html"], first["ast_debug"], first["children"]
            ):
    ...
    for mode in modes:
        ...
        if any(left[key] != right[key] for key in ("html", "ast_debug", "children")):
            raise AssertionError((case["name"], mode, "baseline/candidate output changed"))
    return {"status": "exact", "modes": outputs}


def metric(verification, mode, engine):
    value = verification["modes"][mode][engine]
    return value["children"] if mode == "parse" else len(value["html"].encode("utf-8"))
...
def main():
    ...
    frozen_worker = args.build / "worker.rs"
    if not frozen_worker.is_file() or digest(frozen_worker) != build_data["worker_sha256"]:
        raise SystemExit("frozen worker.rs is missing or changed after prepare.py")
    ...
    for index, case in enumerate(cases, 1):
        ...
        if index % 20 == 0: print(f"  {index}/{len(cases)} verified", flush=True)
    ...
    seed = 20260914
    config = {
        "schema": 2, "rounds": args.rounds, "pairs_per_round": args.pairs,
        "window_ms": args.window_ms, "warmup_ms": max(5, min(50, args.window_ms)),
        "seed": seed, "modes": args.modes, "case_filter": args.case_filter,
        "cases": [case["name"] for case in cases], "corpus_sha256": digest(args.output / "corpus.json"),
        "runner_sha256": digest(Path(__file__)), "worker_sha256": build_data["worker_sha256"],
        "host_before": host(),
    }
    rows, capacities = [], []
    ...
    for round_index in range(args.rounds):
        ...
        for job_index, (case, mode) in enumerate(jobs, 1):
            workers = {engine: Worker(build_data["engines"][engine]["binary"], case["profile"], mode,
                                      [input_paths[case["name"]]]) for engine in ENGINES}
            expected = {engine: metric(verification[case["name"]], mode, engine) for engine in ENGINES}
            try:
                before = {}
                for engine in ENGINES:
                    checked = workers[engine].verify()
                    expected_result(checked, verification[case["name"]]["modes"][mode][engine],
                                    (case["name"], mode, engine, "pre-timing"))
                    before[engine] = checked[0]["arena_capacity_bytes"]
                ...
                for pair in range(args.pairs):
                    ...
                    row = {"round": round_index, "pair": pair, "case": case["name"],
                           "profile": case["profile"], "category": case.get("category"),
                           "mode": mode, "order": list(order)}
                    for engine in order:
                        row[engine] = checked_bench(workers[engine], args.window_ms * 1_000_000, expected[engine])
                    rows.append(row)
                after = {}
                for engine in ENGINES:
                    checked = workers[engine].verify()
                    expected_result(checked, verification[case["name"]]["modes"][mode][engine],
                                    (case["name"], mode, engine, "post-timing"))
                    after[engine] = checked[0]["arena_capacity_bytes"]
                capacities.append({"round": round_index, "case": case["name"], "profile": case["profile"],
                                   "category": case.get("category"), "mode": mode, "before": before, "after": after})
            finally:
                for worker in workers.values(): worker.close()
            if job_index % 20 == 0: print(f"  {job_index}/{len(jobs)} completed", flush=True)
    ...
    (args.output / "arena-capacities.json").write_text(json.dumps(capacities, indent=2) + "\n")
    summary = []
    for case in cases:
        for mode in args.modes:
            selected = [row for row in rows if row["case"] == case["name"] and row["mode"] == mode]
            ratios = [r["baseline"]["elapsed_ns"] / r["baseline"]["iterations"] /
                      (r["candidate"]["elapsed_ns"] / r["candidate"]["iterations"]) for r in selected]
            summary.append({"case": case["name"], "profile": case["profile"], "category": case.get("category"),
                            "mode": mode, "baseline_ns_per_document": statistics.median(
                                r["baseline"]["elapsed_ns"] / r["baseline"]["iterations"] for r in selected),
                            "candidate_ns_per_document": statistics.median(
                                r["candidate"]["elapsed_ns"] / r["candidate"]["iterations"] for r in selected),
                            "baseline_over_candidate_median": statistics.median(ratios),
                            "baseline_over_candidate_min": min(ratios), "baseline_over_candidate_max": max(ratios),
                            "pairs": len(ratios),
                            "round_medians": [statistics.median(
                                r["baseline"]["elapsed_ns"] / r["baseline"]["iterations"] /
                                (r["candidate"]["elapsed_ns"] / r["candidate"]["iterations"])
                                for r in selected if r["round"] == round_index)
                                for round_index in range(args.rounds)]})
