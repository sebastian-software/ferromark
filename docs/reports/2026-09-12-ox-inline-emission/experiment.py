...
def run(name,rounds=5,ms=30,tag=None,order=0,warm=60):
 dest=W/(tag or name);dest.mkdir(exist_ok=True)
 selected=json.loads((W/'selected.json').read_text());cases=json.loads((W/'cases.json').read_text());workers={n:Worker(W/'bin'/n,W/'cases.json') for n in ['baseline',name]};raw=[];summary=[]
 for i in selected:
