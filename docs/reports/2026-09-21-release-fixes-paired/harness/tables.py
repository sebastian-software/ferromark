...
# The corpus is not archived (run.json pins its hash); regenerate it with
# benchmarks/optimization-rounds/make_corpus.py --include-scanner-diagnostics.
CORPUS = __import__("os").environ.get("PAIRED_CORPUS", "/private/tmp/native-release-work/paired-corpus.json")
...
for spec in sys.argv[4:]:
    ...
    corpus = {c["name"]: c for c in json.load(open(CORPUS))["cases"]}
    lines += [f"## {heading}", ""]
