...
cd "$REPO"
rm -rf $P/build-$NAME
python3 $H/prepare.py --baseline-path $P/src-base --candidate-path $REPO --out $P/build-$NAME --baseline-revision c4af9525 --reuse-baseline-build $P/build-head > $P/build-$NAME.log 2>&1 || { echo "BUILD FAILED"; tail -20 $P/build-$NAME.log; exit 1; }
echo "built $NAME"
...
screen build-$NAME $NAME-broad $P/filter-broad.txt 5 fresh reuse parse render
