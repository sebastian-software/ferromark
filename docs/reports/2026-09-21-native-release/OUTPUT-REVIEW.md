...
## The final v2 review changed no output on this corpus

Every one of the 342 archived HTML outputs (57 documents × 6 engines) is
byte-identical to [2026-09-16-native-segments](../2026-09-16-native-segments/README.md),
which measured v2 at `7c887a2b`, and therefore also to the two reports before
it. The agreement classifications are identical too, so the 14-case and 50-case
scored subsets are the same sets of documents. `audit.py` asserts this equality
over the whole `verification.json`, and `compare_previous.py` refuses to emit a
comparison if either the outputs or the agreement sets differ.

This is not a claim that v2 renders every input as it did at `7c887a2b`. The
review deliberately changed output for three shapes, each toward its written
rule: a paired JSX tag at line start now owns its line under `mdx`, a tab after
a list marker is expanded per CommonMark 0.31.2, and a type-1 HTML block ends on
the line that carries its closer
([decision record](../../decisions/2026-09-21-mdx-flow-and-block-conformance.md)).
The renderer's empty `base_url` now keeps root-absolute links
([decision record](../../decisions/2026-09-21-renderer-fixes.md)). None of the 57
frozen inputs contains one of those shapes under the benchmark flags — `mdx` is
off in both lanes and the renderer runs with no `base_url` — so the six-engine
HTML over this corpus is unchanged, which is the independent evidence this
report adds. The cost fixes that motivated the rerun (linear nested-link
probing, memoized MDX closers, linear math/MDX/definition-list scans, bounded
container and inline costs) change no output by design; their own records and
tests carry that argument.
