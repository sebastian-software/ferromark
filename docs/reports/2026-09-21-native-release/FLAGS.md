...
Ferromark v2 is pinned to `bffc89f6` and v1 to `4e15141`; the other engines retain
...
The v2 changes measured here — the final pre-release review's parser and
renderer fixes, the linear nested-link probe, the MDX closer memo and the
linear math/MDX/definition-list scans — add no option and rename none. Math,
MDX and definition lists are off in both lanes; the fixed inline shapes are
core CommonMark syntax and run under the same flags as before. The adapters
and the executable guards are unchanged from the preceding report.
...
flags. The executable guards retain that distinction while forbidding subscript;
literal single tildes are asserted in the CommonMark lane.
...
The held-out PGO comparison uses the same flag contract and the same frozen
inputs. `prepare.py --pgo` changes only `RUSTFLAGS`; no engine option differs
between the default and PGO executables. A PGO row is compared only with PGO
rows, per the harness README.
