...
PREVIOUS = "2026-09-16-native-segments"
REPORT = "docs/reports/2026-09-21-native-release"
DATE = "2026-09-21"
...
"""
    ...
    text = f"""# Native comparison before the 2.0.0 release — {DATE}

{common}

## What changed in v2, and what these numbers may be attributed to

This run measures `main` at `{v2}`, the head of the v2 line after the final
...
{position_table}
**They did.** V2 lost {smallest_move:.0f}–{largest_move:.0f}% of its relative
...
three process rounds of this run agree to within {round_spread:.3f} on every
...
{len(losses)} of the {len(previous)} document/lifecycle rows, so this is not
...
relative to v1, the largest by {largest_loss:.2f}%{' (the twelve largest are listed)' if len(losses) > 12 else ''}.
The largest losses sit on comments with task lists and block quotes and on
technical pages with many list items and fences; the {len(previous) - len(losses)} rows that moved the
other way are within {largest_gain:.1f}%:
...
gaining 1.258× fresh / 1.251× reuse. At `{v2}` the same recipe gains
{gains['v2'][0]:.3f}× / {gains['v2'][1]:.3f}×. The other five engines are unchanged pins; these are
