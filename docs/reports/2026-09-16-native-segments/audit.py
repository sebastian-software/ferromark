...
def main():
    ...
    previous = read(HERE.parent / '2026-09-15-release-native', 'verification.json')
    ...
    for build in (build_default, build_pgo):
        assert build['engines']['ferromark_v2']['revision'].startswith('7c887a2b')
    ...
    result = dict(status='passed', documents=len(cases), workloads=len(jobs),
                  ...
                  checks=['input lengths and hashes', 'all timed checksums', 'complete unique window coverage',
                          'unchanged HTML and agreement classifications against 2026-09-15-release-native',
