...
## The segmented definition pass changed no output

Every one of the 342 archived HTML outputs (57 documents × 6 engines) is
byte-identical to [2026-09-15-release-native](../2026-09-15-release-native/README.md),
which measured v2 at `c232d97d`. The agreement classifications are identical
too, so the 14-case and 50-case scored subsets are the same sets of documents.
`audit.py` asserts this equality over the whole `verification.json`, and
`compare_previous.py` refuses to emit a comparison if either the outputs or
the agreement sets differ.

This is the property the pass was designed to have: it changes which bytes the
block grammar is asked to look at when discovering link reference and footnote
definitions, never what a definition is. The decision record states the
boundary proof
([2026-09-16-segmented-definition-pass](../../decisions/2026-09-16-segmented-definition-pass.md)),
and the differential harness described there checks the two collections for
equality on the specification fixtures, the frozen corpora and generated token
soup. This report adds the independent evidence that six engines' HTML over
the frozen corpus is unchanged.

The two held-out runs reproduce the same outputs: `audit.py` checks every
document's HTML in both the default and the PGO held-out results against the
57-document run.
