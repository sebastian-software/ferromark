...
REPORT = "docs/reports/2026-09-21-native-release-fixed"
DATE = "2026-09-21"
...
"""
    ...
    text = f"""# Native comparison after the release-fix regression was repaired — {DATE}

{common}

## What changed in v2, and what these numbers may be attributed to

This run measures `main` at `{v2}`: the release head `bffc89f6` plus the
regression fix. The [preceding run](../{REGRESSED}/README.md) measured
...
[2026-09-16-native-segments](../{PREVIOUS}/README.md), not the regressed run.
...
{largest_move:.2f}% and the smallest {smallest_move:.2f}%, and against md4c v2 is
ahead of the `7c887a2b` run. The three process rounds of this run agree to
within {round_spread:.3f} on every validation row below, and the other five
...
relative to v1 since the `7c887a2b` run, the largest by {largest_loss:.2f}%; the
...
1.258× fresh / 1.251× reuse; the regressed run 1.287× / 1.285×. At `{v2}` the
same recipe gains {gains['v2'][0]:.3f}× / {gains['v2'][1]:.3f}×. The other five engines are
