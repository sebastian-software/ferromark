...
def run_regimes(name,rounds=5,ms=30,tag='regime-screen'):
 dest=W/name/tag;dest.mkdir(parents=True,exist_ok=True);ids=json.loads((W/'regime-selected.json').read_text());cases=json.loads((W/'cases.json').read_text());workers={n:Worker(W/'bin'/n,W/'cases.json') for n in ['baseline',name]};raw=[];summary=[]
 for i in ids:
