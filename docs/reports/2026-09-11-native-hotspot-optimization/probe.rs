...
fn corpora() -> Vec<(String, String)> {
    ...
    // CommonMark inline parsing stays enabled for all table workloads.
    for (name, fixture) in [
        ("tables-plain", "tables-plain"),
        ("tables-links", "tables-links"),
        ("tables-commonmark-inline", "tables-commonmark-inline"),
    ] {
        result.push((
            name.into(),
...
fn main() {
    ...
    for (lane, flags) in CONFIGURATIONS {
        ...
        for (name, input) in corpora() {
            if ["tables-plain", "tables-commonmark-inline", "tables-links"].contains(&name.as_str())
                && flags != 1
            {
                continue;
            }
