...
from run import PARSERS, mismatches, workload_review

LABELS = {
    "ferromark": "ferromark", "pulldown-cmark": "pulldown-cmark",
    "bun_md": "Bun (native)", "comrak": "comrak", "md4c": "md4c (C)",
}
ORDER = list(LABELS)
LEGACY_CASES = [
    ...
    ("tables/gfm-tables", "tables", "Tables + inactive strikethrough (historical)", "feature"),
...
CASES = [
    ...
    ("commonmark/publication-5k", "commonmark-5k", "CommonMark · 5 KiB", "size"),
    ("commonmark/publication-10k", "commonmark-10k", "CommonMark · 10 KiB", "size"),
    ("tables/tables-plain", "tables-plain", "Tables: plain text", "feature"),
    ("tables/tables-commonmark-inline", "tables-commonmark-inline", "Tables: emphasis and strong", "feature"),
    ("tables/tables-links", "tables-links", "Tables: link column", "feature"),
    ("strikethrough/strikethrough", "strikethrough", "Strikethrough only", "feature"),
    ("commonmark/links", "links", "CommonMark links and images", "feature"),
    ("commonmark/entities", "entities", "CommonMark entities and inline markup", "feature"),
]
DEFAULT_SOURCE = REPO / "docs/reports/2026-09-11-native-optimization-publication/native"
CURRENT_REPORT = "docs/reports/2026-09-11-native-optimization-publication.md"


def publication_cases(metadata):
    # Preserve archived publications without mixing their obsolete inputs into new runs.
    if any(case.endswith("/gfm-tables") for case in metadata["protocol"].get("headline_cases", [])):
        return LEGACY_CASES
    return CASES
...
def load_publication(folder, cases=None):
    metadata = json.loads((folder / "metadata.json").read_text())
    catalog = publication_cases(metadata)
    if cases is None:
        cases = catalog
    ...
    for case, identifier, label, kind in cases:
        ...
        # Publication has a frozen case catalog, independent of future benchmark defaults.
        if case not in {row[0] for row in catalog} or set(verified[case]["output_sha256"]) != PARSERS:
        ...
        winning_ns = min(value[0] for value in values.values())
        rows = []
        for parser in ORDER:
            ...
            rows.append({"parser": LABELS[parser], "latency": f"{ns / 1000:.3f} µs",
                ...
                "medianNs": ns, "winner": ns == winning_ns, "runMediansNs": medians,
...
def data_for(folder):
    catalog = publication_cases(json.loads((folder / "metadata.json").read_text()))
    ...
    if set(overrides) - {case for case, *_ in catalog}:
        raise ValueError("Unknown publication override case")
    metadata, tables, verified = load_publication(folder, [row for row in catalog if row[0] not in overrides])
    for case, directory in overrides.items():
        ...
        corrected_meta, corrected_tables, corrected_verified = load_publication(
            corrected_folder, [row for row in catalog if row[0] == case])
    ...
    tables.sort(key=lambda table: [row[0] for row in catalog].index(table["case"]))
    ...
    return {
        ...
        "source":source, "report": CURRENT_REPORT if catalog == CASES else "docs/reports/2026-09-11-benchmark-refresh.md", "corrections":overrides, "ferromarkRevision":metadata["ferromark_revision"],
...
'''
    for table in data["featureTables"]:
        text += "| " + " | ".join([table["label"], str(table["inputBytes"]), *[f"**{r['latency']}**" if r["winner"] else r["latency"] for r in table["rows"]]]) + " |\n"
    text += f'''
...
def native_report_section(data):
    ...
    for table in [*data["tables"], *data["featureTables"]]:
        for row in table["rows"]:
            if row["winner"]:
                winners.setdefault(row["parser"], []).append(table["label"])
    ...
    for table in [*data["tables"], *data["featureTables"]]:
        for row in table["rows"]:
            ...
            latency = f"**{row['latency']}**" if row["winner"] else row["latency"]
            lines.append(f"| {table['label']} | {row['parser']} | {latency} | {medians} | {row['spreadPercent']:.1f}% | {row['outputBytes']} |")
    return "\n".join(lines)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("folder", type=Path, nargs="?", default=DEFAULT_SOURCE)
