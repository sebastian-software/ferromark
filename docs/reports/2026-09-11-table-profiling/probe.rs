...
use ferromark::{BlockParser, Options, RenderPolicy, Renderer};
#[cfg(feature = "counts")]
use ferromark_pulldown_comparison::{CountingAllocator, MeasurementWindow};
use pulldown_cmark::{Options as POptions, Parser, html};
use serde_json::json;
use std::{
    hint::black_box,
...
fn corpus(name: &str, root: &str) -> String {
    match name {
        ...
        "fixture" => {
            std::fs::read_to_string(format!("{root}/benches/fixtures/tables-5k.md")).unwrap()
        }
        "control" => {
            std::fs::read_to_string(format!("{root}/benches/fixtures/commonmark-5k.md")).unwrap()
        }
...
fn options(config: &str) -> (Options, POptions) {
    ...
    if config == "overlap" {
        ...
        f.task_lists = true;
        p |= POptions::ENABLE_STRIKETHROUGH | POptions::ENABLE_TASKLISTS;
    }
    assert!(config == "tables" || config == "overlap");
    (f, p)
}
// The runtime guard rejects timing requests in the instrumented binary.
...
fn main() {
    ...
    let mut render = || -> usize {
        match lane.as_str() {
            "ferro-owned" => {
                let out = ferromark::to_html_with_options(black_box(&input), &f);
                black_box(&out);
                out.len()
            }
            "pull-owned" => {
                let mut out = String::new();
                html::push_html(&mut out, Parser::new_ext(black_box(&input), p));
                black_box(&out);
                out.len()
            }
            "ferro-reuse" => {
                ferromark::to_html_into_with_options(black_box(&input), &mut output, &f);
                black_box(&output);
                output.len()
            }
            "pull-reuse" => {
                pout.clear();
                html::push_html(&mut pout, Parser::new_ext(black_box(&input), p));
                black_box(&pout);
                pout.len()
            }
            "ferro-retained" => {
            ...
            "ferro-block" => {
                events.clear();
                BlockParser::new_with_options(black_box(input.as_bytes()), f.clone())
                    .parse(&mut events);
                black_box(&events);
                events.len()
            }
            "pull-events" => {
                let mut n = 0;
                for event in Parser::new_ext(black_box(&input), p) {
                    black_box(event);
                    n += 1;
                }
                n
    ...
    // Keep instrumented binaries from silently producing benchmark timings.
    ...
    loop {
        for _ in 0..16 {
            bytes = black_box(render());
            iterations += 1;
        }
        if mode != "forever" && start.elapsed() >= duration {
            break;
        }
    }
    println!(
        "{}",
        json!({"lane":lane,"corpus":name,"config":config,"input_bytes":input.len(),"output_bytes":bytes,"iterations":iterations,"elapsed_ns":start.elapsed().as_nanos()})
    );
}
