 ...
 for line in io::stdin().lock().lines(){let q:Value=serde_json::from_str(&line.unwrap()).unwrap();let i=q["index"].as_u64().unwrap() as usize;let c=&cases[i];let o=&opts[i];
  let row=match q["op"].as_str().unwrap(){
   ...
   "blocks"=>{let mut p=ferro::BlockParser::new_with_options(c.input.as_bytes(),o.clone());let mut events=vec![];p.parse(&mut events);
    ...
    json!({"events":format!("{events:?}"),"html":r.html,"headings":format!("{:?}",r.headings),"limits":format!("{:?}",r.resource_limits),"mdx_events":format!("{:?}",ferro::mdx::parse_events(&c.input)),"segments":format!("{:?}",ferro::mdx::segment_spanned(&c.input)),"mdx_body":mdx.body,"mdx_esm":mdx.esm,"mdx_front_matter":mdx.front_matter})},
   ...
   "window-single"=>{let ms=q["ms"].as_u64().unwrap();let start=Instant::now();let mut n=0;loop{for _ in 0..1{if c.flags&256!=0{drop(black_box(ferro::mdx::render_with_options(black_box(&c.input),o)));}else if c.reuse{drop(black_box(reuse[i].render(black_box(&c.input))));}else{drop(black_box(ferro::to_html_with_options(black_box(&c.input),o)));}}n+=1;if start.elapsed()>=Duration::from_millis(ms){break;}}
    json!({"case":c.case,"count":n,"elapsed_ns":start.elapsed().as_nanos() as u64})},
   "escape"=>{let mut escaped=Vec::with_capacity(c.input.len());if c.flags&1!=0 {ferro::escape_attr_into(&mut escaped,c.input.as_bytes());}else{ferro::escape_text_into(&mut escaped,c.input.as_bytes());}json!({"output":escaped})},
   "window-escape"|"window-escape-single"=>{let ms=q["ms"].as_u64().unwrap();let batch=if q["op"]=="window-escape-single"{1}else{16};let start=Instant::now();let mut n=0;loop{for _ in 0..batch{let mut escaped=Vec::with_capacity(c.input.len()*if c.flags&2!=0{2}else{1}+if c.flags&4!=0{16}else{0});if c.flags&1!=0{ferro::escape_attr_into(&mut escaped,black_box(c.input.as_bytes()));}else{ferro::escape_text_into(&mut escaped,black_box(c.input.as_bytes()));}drop(black_box(escaped));}n+=batch;if start.elapsed()>=Duration::from_millis(ms){break;}}json!({"case":c.case,"count":n,"elapsed_ns":start.elapsed().as_nanos() as u64})},
   _=>panic!("unknown op"),
