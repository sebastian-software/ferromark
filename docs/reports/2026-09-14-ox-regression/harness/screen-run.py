...
def main():
        ...
        for label in variants:
            ...
            for mode in args.modes:
                ...
                try:
                    ...
                    w.close()
    # Each engine/variant must agree across stages, including full AST Debug
    # for every stage exposing it. Optional candidate variants are checked
    # against their control's AST in the caller's archived validation.
    for name, engines in verification.items():
        for label, stages in engines.items():
            assert len({v['html'] for v in stages.values()}) == 1, (name, label)
            assert len({v['ast_debug'] for v in stages.values() if 'ast_debug' in v}) <= 1
    ...
    config = {'rounds': args.rounds, 'pairs': args.pairs, 'window_ms': args.window_ms,
              'modes': args.modes, 'seed': 20260914, 'host_before': native.host(),
