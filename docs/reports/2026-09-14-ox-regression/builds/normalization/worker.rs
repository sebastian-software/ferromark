...
fn main() {
    v2::Parser::set_diagnostic_skip_normalization(
        std::env::var_os("FERROMARK_DIAGNOSTIC_SKIP_NORMALIZATION").is_some(),
    );
    let args: Vec<String> = std::env::args().skip(1).collect();
    ...
    let sources: Vec<String> = args[3..].iter().map(|p| std::fs::read_to_string(p).unwrap()).collect();
    match args[0].as_str() {
