...
#[test]
fn normalizes_void_checkboxes_without_removing_attributes() {
    assert_eq!(
        normalize_html("<input checked disabled type=checkbox />"),
        normalize_html("<input type=checkbox disabled checked>")
    );
    assert_ne!(
        normalize_html("<input checked type=checkbox>"),
        normalize_html("<input type=checkbox>")
    );
}
