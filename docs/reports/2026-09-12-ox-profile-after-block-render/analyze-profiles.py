...
def parse(path):
 ...
 total=sum(n['self'] for n in nodes);self_counts=collections.Counter();lines=collections.Counter();inclusive=collections.Counter();stage=collections.Counter();alloc=0;heading=0;sorting=0;conversion=0;utf8=0
 for i,n in enumerate(nodes):
  ...
  if 'into_string_with_ascii_prefix' in joined:conversion+=n['self']
  if 'core::str::converts::from_utf8' in joined:utf8+=n['self']
  ...
  elif 'corpus_ox::engine::render' in joined:label='remaining Ox pipeline/runtime'
