...
def timing(tag,selected=None,rounds=3,ms=8):
 cases=json.loads((W/'case-manifest.json').read_text());verified=json.loads((W/'verification.json').read_text());workers={n:Worker(n) for n in ['ferro','ox-grow']};raw=[];summary=[]
 ...
 for i in indices:
  ...
  for r in range(rounds):
   names=list(workers);names=names[r%len(names):]+names[:r%len(names)]
   for n in names:
