...
lines=['# HTML line scanning measurements','','Generated from the archived measurement records. Negative changes mean less elapsed time. Exploratory screens and independent production confirmations are reported separately.','','## Profiles','','Two six-second native sample captures per document/implementation at one-millisecond intervals. Visible stack shares are diagnostic, not elapsed-time improvements.','']
ps=read('profile-summary.json');rows=[]
for name in ['probe','baseline','production']:
...
for d in sorted(W.iterdir()):
 ...
 r=read(d.name+'/summary.json');v=read(d.name+'/verification.json');b=read(d.name+('/block-verification.json' if (d/'block-verification.json').exists() else '/timing-verification.json'))
