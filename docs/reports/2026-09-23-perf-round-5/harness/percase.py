#!/usr/bin/env python3
"""Side-by-side per-case ratios of several runs: percase.py <run-name>...  (e.g. trim-broad total-broad)"""
import json
import sys
...
print(f"{'mode':7s} {'case':44s} " + " ".join(f"{n:>17s}" for n in names))
for k in keys:
    row = []
    for n in names:
        r = data[n].get(k)
        row.append(f"{r['baseline_over_candidate_median']:17.3f}" if r else f"{'-':>17s}")
