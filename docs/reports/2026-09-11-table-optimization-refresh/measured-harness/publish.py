...
def load_publication(folder, cases=CASES):
    ...
    for case, identifier, label, kind in cases:
        ...
        if case not in {row[0] for row in CASES} or set(verified[case]["output_sha256"]) != PARSERS:
...
'''
    ...
    text += f'''
The historical inactive-strikethrough and GFM-subset rows use identical mixed input.
The first measures tables plus literal inactive syntax, not isolated table cost.
New dedicated `tables/tables-plain`, `tables/tables-commonmark-inline`, and
`tables/tables-links` benchmarks
compare plain cells, emphasis/strong cells, and a link column with CommonMark
always active; the
[focused remeasurement](docs/reports/2026-09-11-commonmark-table-costs.md) records
their current Ferromark/pulldown comparison. The GFM-subset row also enables
