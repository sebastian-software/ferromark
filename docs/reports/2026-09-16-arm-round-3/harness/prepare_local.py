...
def verify_baseline(source: Path):
    ...
    repository = Path(os.environ.get("FMV2_REPO", Path(__file__).resolve().parents[2]))
...
def build_engine(name, source: Path, build: Path, worker: bytes, worker_sha: str, lto: str, cached):
    ...
    return {
        ...
        "source_lock_sha256": sha256(source / "Cargo.lock"),
        "worker_sha256": worker_sha, "lto": lto, "symbols": SYMBOLS, "rustflags": env["RUSTFLAGS"],
...
def main():
    global BASELINE_REVISION, SYMBOLS
    ...
    parser.add_argument("--symbols", action="store_true", help="keep symbols and line tables for profiling")
    args = parser.parse_args()
    BASELINE_REVISION = subprocess.check_output(["git", "-C", os.environ.get("FMV2_REPO", str(Path(__file__).resolve().parents[2])), "rev-parse", args.baseline_revision], text=True).strip()
