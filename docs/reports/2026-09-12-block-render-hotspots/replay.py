...
for source in (args.work / "production-source").rglob("*.rs"):
    name = str(source.relative_to(args.work / "production-source"))
    if hashlib.sha256(source.read_bytes()).hexdigest() != metadata["production_source_hashes"][name]:
        raise SystemExit(f"Production source identity mismatch: {name}")
...
if args.verify:
    ...
    for script, oracle in [
        ("verify-blocks.py", "block-hashes.json"),
        ("verify-fences.py", "fence-hashes.json"),
        ("verify-timing.py", "timing-hashes.json"),
        ("verify-extra.py", "extra-hashes.json"),
        ("verify-extended.py", "extended-hashes.json"),
    ]:
        oracle_path = args.work / "baseline" / oracle
        original_oracle = json.loads(oracle_path.read_text())
        subprocess.run([sys.executable, str(args.work / script), "baseline"], check=True)
        if json.loads(oracle_path.read_text()) != original_oracle:
            raise SystemExit(f"Rebuilt baseline differs from the archived oracle: {oracle}")
    if not build("production") or not verify("production"):
        raise SystemExit("Production verification failed")
    for script, record in [
        ("verify-blocks.py", "block-verification.json"),
        ("verify-fences.py", "fence-verification.json"),
        ("verify-timing.py", "timing-verification.json"),
