# ARCH-COMP-001: md4c vs pulldown-cmark vs ferromark
...
| Area | md4c | pulldown-cmark | ferromark | Notes |
|---|---|---|---|---|
...
| Entity handling | Decode entities at render time; parsing aware of `&`. `4/5` | Entity decoding where required. `4/5` | Decode entities in label normalization and renderer; fast-path when no `&`. `4/5` | ferromark already optimized; ensure behavior parity. |
