...
fn prose_expressions_do_not_emit_javascript() {
    ...
    assert!(
        ...
        "expressions are not islands:\n{html}"
    );
    assert!(
        !html.contains("{name}"),
        "expression source is not leaked as text:\n{html}"
    );
    assert!(
        !html.contains("{count + 1}"),
        "flow source is not leaked as text:\n{html}"
    );
}
...
fn hostile_expression_does_not_emit_raw_markup() {
    ...
    assert!(
        !html.contains("<script"),
        "hostile source must not render as HTML:\n{html}"
    );
    assert!(
        !html.contains("alert(1)"),
        "expression source is not evaluated or emitted:\n{html}"
    );
}
...
fn esm_is_metadata_only_in_static_html() {
    ...
    assert!(
        !html.contains("import Chart"),
        "imports must not leak into HTML:\n{html}"
    );
    assert!(
        !html.contains("export const"),
        "exports must not leak into HTML:\n{html}"
    );
    assert!(
        !html.contains("<script"),
        "ESM must not become executable HTML:\n{html}"
    );
    assert!(
