...
pub(in crate::renderer::html) struct FirstByteIndex {
    table: [bool; 256],
    needles: [u8; 3],
    needle_len: usize,
    overflow: bool,
...
impl FirstByteIndex {
    ...
    pub(in crate::renderer::html) fn may_match(&self, haystack: &[u8]) -> bool {
    ...
    #[inline]
    pub(in crate::renderer::html) fn rejects_second(&self, first: u8, after: Option<&u8>) -> bool {
    ...
    #[inline]
    pub(in crate::renderer::html) fn next(&self, hay: &[u8]) -> Option<usize> {
        if self.overflow {
