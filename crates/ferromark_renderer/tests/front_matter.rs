...
fn yaml_is_extracted_and_omitted_from_html() {
    ...
    assert_eq!(front_matter.span.source_text(source), "---\ntitle: Hello\n---\n");
    assert_eq!(front_matter.content_span.source_text(source), front_matter.value);
...
fn renderer_hooks_and_incremental_paths_omit_front_matter() {
    ...
    let body_document =
        Parser::with_options(&body_allocator, "# Content\n", ParserOptions::gfm()).parse().unwrap();
    let mut baseline_renderer = HtmlRenderer::with_options(HtmlRendererOptions::gfm());
    let mut baseline_hooks = Hooks { nodes: 0 };
    assert_eq!(baseline_renderer.render_with_hooks(&body_document, &mut baseline_hooks), expected);
    ...
    let mut hooks = Hooks { nodes: 0 };
    assert_eq!(renderer.render_incremental_fragment_with_hooks(&document, &mut hooks), expected);
    assert_eq!(hooks.nodes, baseline_hooks.nodes);
...
fn toml_and_all_supported_line_endings_preserve_value_bytes() {
    for (source, value, closing_end) in [
        ("+++\ntitle = \"Hello\"\n+++\n# Content", "title = \"Hello\"\n", 24),
        ("+++\r\ntitle = \"Hello\"\r\n+++\r\n# Content", "title = \"Hello\"\r\n", 27),
        ("+++\rtitle = \"Hello\"\r+++\r# Content", "title = \"Hello\"\r", 24),
    ] {
        let allocator = Allocator::new();
        let document = parse(&allocator, source);
        let front_matter = document.front_matter.as_ref().expect("front matter");
...
fn empty_front_matter_and_eof_closing_delimiter_are_supported() {
    for source in ["---\n---\nContent", "---\ntitle: x\n---"] {
        ...
        assert_eq!(
            front_matter.value,
            if source == "---\n---\nContent" { "" } else { "title: x\n" }
        );
        assert_eq!(front_matter.content_span.len(), front_matter.value.len() as u32);
    }
}
...
fn front_matter_is_disabled_in_every_parser_preset_by_default() {
    for parser_options in [
        ...
        let allocator = Allocator::new();
        let document =
            Parser::with_options(&allocator, source, parser_options.clone()).parse().unwrap();
        assert!(document.front_matter.is_none());
        assert_eq!(render(source, parser_options), "<hr>\n<h2>title: Hello</h2>\n");
    }
}
...
fn invalid_openers_closers_and_prefixes_fall_back_to_markdown() {
    for source in [
        ...
        let allocator = Allocator::new();
        let document = parse(&allocator, source);
        assert!(document.front_matter.is_none(), "unexpected front matter in {source:?}");
        assert_eq!(render_with_front_matter(source), render(source, ParserOptions::gfm()));
    }
}
...
fn delimiter_lines_allow_only_ascii_space_and_tab_after_three_markers() {
        ...
        ["---  \ntitle: Hello\n---\t\n# Content", "+++  \ntitle = \"Hello\"\n+++\t\n# Content"]
    {
        let allocator = Allocator::new();
        let document = parse(&allocator, source);
        assert!(document.front_matter.is_some(), "expected front matter in {source:?}");
    }
...
fn value_is_an_exact_borrowed_slice_and_can_contain_nul() {
    ...
    assert_eq!(
        ...
        source.as_ptr().wrapping_add(front_matter.content_span.start as usize)
    );
    assert_eq!(front_matter.content_span.source_text(source), front_matter.value);
    assert_eq!(
...
fn a_second_front_matter_looking_block_is_ordinary_body_markdown() {
    ...
    assert_eq!(render_with_front_matter(source), render(body, ParserOptions::gfm()));
}
...
fn a_bom_after_front_matter_is_body_content_and_is_not_stripped_again() {
    ...
    assert_eq!(render_with_front_matter(source), "<p>\u{feff}# Content</p>\n");
    assert_eq!(document.span, Span::new(0, source.len() as u32));
}
...
fn metadata_is_opaque_to_markdown_extensions_and_definitions() {
    ...
    assert_eq!(html, "<p>Use [ref] and [^note] and <a href=\"/body\">body</a>.</p>\n");
...
fn front_matter_is_not_extracted_after_comments_or_inside_containers() {
    for source in [
        ...
        let enabled = ParserOptions { line_comments: true, mdx: true, ..options() };
        let disabled = ParserOptions { front_matter: false, ..enabled.clone() };
        let allocator = Allocator::new();
        let document = Parser::with_options(&allocator, source, enabled.clone()).parse().unwrap();
        assert!(document.front_matter.is_none(), "unexpected metadata in {source:?}");
        assert_eq!(render(source, enabled), render(source, disabled), "{source:?}");
    }
}
