...
fn render(source: &str) -> String {
    ...
    let document = Parser::with_options(
        &allocator,
        source,
        ParserOptions { merged_table_cells: true, ..ParserOptions::gfm() },
    )
    .parse()
    .unwrap();
...
fn column_classes(html: &str) -> Vec<&str> {
    html.split("<col class=\"").skip(1).map(|part| part.split('"').next().unwrap()).collect()
}
...
fn blank_headers_fall_back_and_numeric_headers_cannot_alias_positions() {
    ...
    assert_eq!(
        column_classes(&html),
        ["col-1", "col-2", "col-3 col-name-2", "col-4 col-name-section"]
    );
}
...
fn duplicate_names_and_natural_suffixes_never_collide() {
    ...
    assert_eq!(
        column_classes(&html),
        ["col-1 col-name-price-1", "col-2 col-name-price", "col-3 col-name-price-2"]
    );
}
...
fn merged_headers_name_each_covered_logical_column() {
    ...
    assert_eq!(
        column_classes(&html),
        ["col-1 col-name-group", "col-2 col-name-group", "col-3 col-name-group-1"]
    );
}
...
fn naming_is_table_local_and_does_not_change_heading_ids_or_renderer_reuse() {
    let source = "# Price\n\n| Price | Price |\n| --- | --- |\n\n# Price\n\n| Price | Price |\n| --- | --- |";
    let allocator = Allocator::new();
    let document = Parser::with_options(&allocator, source, ParserOptions::gfm()).parse().unwrap();
    let opts = HtmlRendererOptions { heading_ids: true, ..options() };
    ...
    assert_eq!(renderer.render_with_hooks(&document, &mut NoHtmlRenderHooks), html);
}
...
fn names_are_opt_in_and_require_colgroup() {
    ...
    let names_without_columns =
        HtmlRendererOptions { table_column_names: true, ..HtmlRendererOptions::gfm() };
    assert_eq!(HtmlRenderer::with_options(names_without_columns).render(&document), plain);
    let columns_only = HtmlRendererOptions { table_colgroup: true, ..HtmlRendererOptions::gfm() };
    assert_eq!(
...
fn named_columns_keep_xhtml_and_hook_output_identical() {
    ...
    let allocator = Allocator::new();
    let document = Parser::with_options(&allocator, source, ParserOptions::gfm()).parse().unwrap();
    let mut renderer = HtmlRenderer::with_options(HtmlRendererOptions { xhtml: true, ..options() });
    let html = renderer.render(&document);
