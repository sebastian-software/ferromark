...
fn script_payload(html: &str) -> Option<Value> {
    ...
    Some(serde_json::from_str(&rest[..end]).unwrap_or_else(|error| {
        panic!(
            "island script is not JSON ({error}): {:?}\n{html}",
            &rest[..end]
        )
    }))
}
...
fn island_inner<'a>(html: &'a str, name: &str) -> &'a str {
    ...
    let start = html
        .find(&marker)
        .unwrap_or_else(|| panic!("missing island {name}:\n{html}"));
    let after_gt = html[start..]
        .find('>')
        .unwrap_or_else(|| panic!("unclosed island tag {name}:\n{html}"));
...
fn page_without_islands_stays_js_free() {
    ...
    assert!(
        ...
        "pages without islands stay JS-free:\n{html}"
    );
    assert!(
        !html.contains("data-ox-island"),
        "prose is not an island:\n{html}"
    );
    assert!(
        !html.contains("ox-island"),
        "no island runtime markers:\n{html}"
    );
}
...
fn island_literal_props_serialize() {
    ...
    assert!(
        html.contains("data-ox-island=\"Alert\""),
        "expected an Alert island:\n{html}"
    );
    let payload = island_payload(&html);
    assert_eq!(
        payload["props"]["title"], "hi",
        "quoted string is a JSON string:\n{html}"
    );
    assert_eq!(
        payload["props"]["count"], 42,
        "{{42}} is a JSON number:\n{html}"
    );
    assert_eq!(
        payload["props"]["disabled"], true,
        "boolean attr is true:\n{html}"
    );
}
...
fn island_expression_stores_source_and_does_not_eval() {
    ...
    let payload = island_payload(&html);
    assert_eq!(
        payload["expressions"]["title"], "foo",
        "expression source is stored:\n{html}"
    );
    assert_eq!(
...
fn hostile_expression_source_cannot_break_out() {
    ...
    assert!(
        ...
        "raw script breakout must not appear:\n{html}"
    );
    assert!(
        !html.contains("<script>alert"),
        "alert must not become executable HTML:\n{html}"
    );
    let payload = island_payload(&html);
    let title = payload["props"]
        .get("title")
        .or_else(|| payload["expressions"].get("title"));
    assert_eq!(
        title,
...
fn mdx_false_does_not_emit_islands() {
    let html = render_with(
        "<Alert title=\"hi\" count={42} {...props} />\n",
        ParserOptions::default(),
    );
    assert!(
        !html.contains("data-ox-island"),
        "mdx=false must not emit islands:\n{html}"
    );
    assert!(
        !html.contains("data-ox-props"),
        "mdx=false must not emit a payload:\n{html}"
    );
    assert!(
        !html.contains("type=\"application/json\""),
        "mdx=false stays JS-free:\n{html}"
    );
}
...
fn disabled_mdx_does_not_parse_jsx_children() {
    ...
    assert!(
        !html.contains("data-ox-island"),
        "mdx=false must not wrap children:\n{html}"
    );
    assert!(
        !html.contains("ox-island"),
        "mdx=false has no island chrome:\n{html}"
    );
    assert!(
        html.contains("<h1"),
        "heading still renders as document HTML:\n{html}"
    );
    assert!(
        html.contains("<strong>world</strong>"),
        "strong is not dropped:\n{html}"
    );
}
...
fn flow_component_renders_heading_and_strong_inside_island() {
    ...
    assert!(
        ...
        "expected a Callout island:\n{html}"
    );
    let inner = island_inner(&html, "Callout");
    assert!(
        inner.contains("<h1"),
        "heading must render inside the island:\n{html}"
    );
    assert!(
        inner.contains("id=\"title\""),
        "heading id is preserved:\n{html}"
    );
    assert!(
    ...
    assert!(
        inner.contains("Hello"),
        "prose must not be dropped:\n{html}"
    );
    let after = html.rsplit_once("</div>").map_or("", |(_, rest)| rest);
    assert!(
        !after.contains("<h1"),
        "heading must not leak after the island:\n{html}"
    );
    assert!(
        !after.contains("<strong>"),
        "strong must not leak after the island:\n{html}"
    );
}
...
fn nested_list_inside_component() {
    ...
    assert!(
        inner.contains("<ul>"),
        "list must render inside the island:\n{html}"
    );
    assert!(
        inner.contains("<li>parent"),
        "parent item is inside the island:\n{html}"
    );
    assert!(
        inner.contains("<li>child"),
        "nested item is inside the island:\n{html}"
    );
    assert!(
        inner.contains("<li>sibling"),
        "sibling item is inside the island:\n{html}"
    );
...
fn fence_inside_component_is_code_not_island() {
    ...
    assert!(
        inner.contains("<pre>"),
        "fence must become a code block:\n{html}"
    );
    assert!(
        inner.contains("language-js"),
        "fence language is preserved:\n{html}"
    );
    assert!(
        inner.contains("const x"),
        "fence text is not dropped:\n{html}"
    );
    assert!(
        !inner.contains("data-ox-island=\"Alert\""),
        "fence JSX is not an island:\n{html}"
    );
    assert!(
...
fn nested_jsx_inside_markdown_children() {
    ...
    assert!(
        inner.contains("<h1"),
        "markdown heading stays inside Callout:\n{html}"
    );
    assert!(
        inner.contains("data-ox-island=\"Badge\""),
        "nested PascalCase is an island:\n{html}"
    );
    assert!(
        html.contains("data-ox-island=\"Callout\""),
    ...
    assert_eq!(
        island_payload(&html)["props"]["title"],
        "hi",
        "nested props still parse:\n{html}"
    );
}
...
fn hostile_child_text_escaped() {
    ...
    let inner = island_inner(&html, "Callout");
    assert!(
        inner.contains("alert(1)"),
        "hostile child text is not dropped:\n{html}"
    );
    assert!(
    ...
    assert!(
        !nested.contains("<script>"),
        "list child script is not raw:\n{nested}"
    );
    assert!(
...
fn unclosed_component_does_not_swallow_file() {
    ...
    assert!(
        !html.contains("data-ox-island"),
        "unclosed opener is not an island:\n{html}"
    );
    assert!(
        html.contains("<h1"),
        "heading after the opener still renders:\n{html}"
    );
    assert!(
        html.contains("Still here"),
        "heading text is not swallowed:\n{html}"
    );
    assert!(
        html.contains("After the unclosed tag."),
...
fn page_without_components_still_js_free() {
    ...
    assert!(
        !prose.contains("<script"),
        "prose pages stay JS-free:\n{prose}"
    );
    assert!(
        !prose.contains("data-ox-island"),
        "prose is not an island:\n{prose}"
    );
    ...
    assert!(
        !fragment.contains("data-ox-island"),
        "nameless wrappers emit no island:\n{fragment}"
    );
    assert!(
        fragment.contains("<h1"),
        "fragment markdown children still render:\n{fragment}"
    );
    assert!(
