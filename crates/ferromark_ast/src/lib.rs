//! AST definitions for Ferromark Markdown parser.
//!
...
///
/// [`ferromark_allocator::Vec`] never runs its elements' destructors, which
...
/// [`ferromark_allocator::Box`], which is a thin pointer with no destructor;
