...
pub struct ParserOptions {
    ...
    /// becomes [`ferromark_ast::Heading::id`] and
    /// [`ferromark_ast::Heading::classes`] instead of rendered text.
    ///
    /// Default: `false`; not enabled by [`ParserOptions::gfm`].
    ...
    /// [`ferromark_ast::Link`] with the raw target stored in
    /// [`ferromark_ast::Link::url`]. The renderer then sees the construct
    ...
    /// Module-level `import` / `export` parse as [`ferromark_ast::MdxjsEsm`].
