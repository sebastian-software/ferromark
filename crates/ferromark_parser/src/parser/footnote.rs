...
use ferromark_ast::{FootnoteDefinition, Node, Span};
...
fn dedent_body<'a>(
    ...
    let mut out = ferromark_allocator::String::with_capacity_in(body.len(), allocator.bump());
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn try_parse_footnote_reference(
        ...
        content: &'a str,
        offset: usize,
        children: &mut ferromark_allocator::Vec<'a, Node<'a>>,
        pos: &mut usize,
    ) -> bool {
