...
fn merged_table_cells_cover_header_and_body_logical_columns() {
    let allocator = Allocator::new();
    let options = ParserOptions { merged_table_cells: true, ..ParserOptions::gfm() };
    ...
    assert_eq!(table.align.len(), 3);
    assert_eq!(
        table.children[0].children.iter().map(|cell| cell.colspan).collect::<Vec<_>>(),
        [2, 1]
    );
    assert_eq!(
        table.children[1].children.iter().map(|cell| cell.colspan).collect::<Vec<_>>(),
        [2, 1]
    );
}
...
fn merged_table_cells_keep_whitespace_empty_cells_and_escaped_pipes() {
    ...
    assert_eq!(body.children.iter().map(|cell| cell.colspan).collect::<Vec<_>>(), [1, 1, 1]);
...
fn merged_table_cells_clamp_and_pad_ragged_rows() {
    ...
    assert_eq!(table.children[1].children.iter().map(|cell| cell.colspan).collect::<Vec<_>>(), [3]);
    assert_eq!(
        table.children[2].children.iter().map(|cell| cell.colspan).collect::<Vec<_>>(),
        [2, 1]
    );
}
