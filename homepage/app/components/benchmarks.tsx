...
export const benchmarkCompetitors = `pulldown-cmark ${benchmarks.competitors["pulldown-cmark"]}, comrak ${benchmarks.competitors.comrak}, md4c @ ${benchmarks.competitors.md4c}, Bun @ ${benchmarks.competitors.bun}`;
...
export const headlineThroughput = benchmarkThroughput(
  benchmarks.headline.table,
  benchmarks.headline.parser,
);
...
/** Independent workloads; columns compare parsers, rows do not price individual syntax. */
export function FeatureBenchmarkTable() {
  return (
    <div className="feature-benchmark-table">
      <table>
        <thead>
          <tr>
            <th>Input / feature set</th>
            <th>Bytes</th>
            {benchmarks.featureTables[0].rows.map((row) => (
              <th key={row.parser}>{row.parser}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {benchmarks.featureTables.map((table) => (
            <tr key={table.id}>
