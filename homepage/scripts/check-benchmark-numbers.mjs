...
/** Every three-column pipe table in document order, cells stripped of Markdown emphasis. */
...
if (!section) {
  ...
  for (const [name, version] of Object.entries(competitors)) {
    const expected = name === "md4c" ? `md4c @ ${version}` : `${name} ${version}`
    check(contains(section, expected), `README benchmarks section must state ${JSON.stringify(expected)}`)
  }
  for (const [label, sentence] of [
    ...
    check(contains(section, sentence), `README benchmarks ${label} must read ${JSON.stringify(normalize(sentence))}`)
  }

  const readmeTables = pipeTables(section).filter((table) => table.header[0]?.toLowerCase() === "parser")
  check(
  ...
  tables.forEach((table, index) => {
    ...
    table.rows.forEach((row, rowIndex) => {
      ...
      const expected = [row.parser, row.throughput, row.ratio]
      const actual = readmeRow.slice(0, 3)
      check(
