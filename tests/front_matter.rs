...
use ferromark::ast::{Document, FrontMatter, FrontMatterKind, Span, Visit};
use ferromark::parser::{Parser, ParserOptions};
use ferromark::renderer::{HtmlRenderControl, HtmlRenderHooks, HtmlRenderer, HtmlRendererOptions};
...
fn renderer_hooks_and_incremental_paths_omit_front_matter() {
    ...
    impl HtmlRenderHooks for Hooks {
        fn render_node(
            &mut self,
            _node: &ferromark::ast::Node<'_>,
            _cx: &mut ferromark::renderer::HtmlRenderContext<'_>,
        ) -> HtmlRenderControl {
            self.nodes += 1;
...
fn body_node_spans_keep_original_coordinates_after_prefix() {
    ...
    impl Visit<'_> for StrongSpans<'_> {
        fn visit_strong(&mut self, strong: &ferromark::ast::Strong<'_>) {
...
fn visitor_sees_front_matter_once_before_body() {
    ...
    impl Visit<'_> for Events<'_> {
        ...
        fn visit_heading(&mut self, _heading: &ferromark::ast::Heading<'_>) {
            self.0.push("heading");
        }
    }
