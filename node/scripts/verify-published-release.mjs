...
for (const name of [...Object.keys(pkg.optionalDependencies), pkg.name]) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    if (response.ok) {
      ...
      // Registry propagation needs bounded retries inside the package loop.
  ...
  if (prerelease)
    assert.equal(metadata["dist-tags"]?.latest, before[name], `${name}: RC changed latest`);
}
...
try {
  ...
  writeFileSync(
    ...
    `import { toHtml } from 'ferromark'; if (toHtml('Hello, **world**!') !== '<p>Hello, <strong>world</strong>!</p>\\n') throw new Error('Unexpected output');`,
  );
  ...
  writeFileSync(
    join(consumer, "smoke.rs"),
    'fn main() { assert_eq!(ferromark::to_html("Hello, **world**!").unwrap(), "<p>Hello, <strong>world</strong>!</p>\\n"); }',
  );
  run("cargo", ["run"]);
