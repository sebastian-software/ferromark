...
export function publishArguments(archive, version) {
  return ['publish', archive, '--access', 'public', '--provenance', '--tag', releaseChannel(version).tag]
}
...
export async function preparePackages(directory, version, { request = fetch } = {}) {
  ...
  assert.deepEqual(readdirSync(directory).filter(name => name.endsWith('.tgz')).sort(), [...expected].sort())
  const packages = []
  for (const [index, name] of names.entries()) {
    const archive = resolve(directory, expected[index])
    const manifestResult = spawnSync('tar', ['-xOzf', archive, 'package/package.json'], { encoding: 'utf8' })
    ...
    const response = await request(`https://registry.npmjs.org/${encodeURIComponent(name)}/${encodeURIComponent(version)}`, { signal: AbortSignal.timeout(30_000) })
    if (response.ok) {
      const metadata = await response.json()
      const integrity = `sha512-${createHash('sha512').update(readFileSync(archive)).digest('base64')}`
      assert.equal(metadata.dist?.integrity, integrity, `${name}: existing version differs from verified archive`)
    } else {
      assert.equal(response.status, 404, `${name}: registry preflight failed`)
    }
    packages.push({ name, archive, exists: response.ok })
  }
  return packages
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const [, , directory, version, mode] = process.argv
  if (!directory || !version || (mode && mode !== '--check') || process.argv.length > 5) {
    throw new Error('Usage: node scripts/publish-packages.mjs <archive-directory> <version> [--check]')
  }
