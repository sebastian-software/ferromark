...
if (args.length > 1 || (args.length === 1 && args[0] !== "--all-targets")) {
  throw new Error(`Usage: node ${path.basename(import.meta.filename)} [--all-targets]`);
}

const packageJson = JSON.parse(await readFile(path.join(packageDir, "package.json"), "utf8"));
const platformTargets = Object.keys(packageJson.optionalDependencies).map((name) =>
  name.slice(`${packageJson.name}-`.length),
);
