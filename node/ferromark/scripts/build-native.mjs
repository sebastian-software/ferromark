...
const corpusArchive = path.join(repoRoot, 'docs', 'reports', '2026-09-14-simd-round', 'corpus.json.gz')
...
async function collectProfile() {
  ...
  const raw = (await readdir(profrawDir)).filter(name => name.endsWith('.profraw'))
  if (raw.length === 0) {
...
async function writeCorpus(corpusDir) {
  ...
  await Promise.all(corpus.cases.map((entry) => {
...
function setRustflags(env, flags) {
  for (const directory of [repoRoot, path.join(repoRoot, 'node'), path.join(repoRoot, 'node', 'native'), packageDir]) {
    for (const name of ['config.toml', 'config']) {
      const file = path.join(directory, '.cargo', name)
      if (existsSync(file) && /^\s*rustflags\s*=/m.test(readFileSync(file, 'utf8'))) {
        throw new Error(`${file} sets rustflags; merge them into build-native.mjs before building with FERROMARK_PGO=1`)
      }
    }
  }
  const encoded = process.env.CARGO_ENCODED_RUSTFLAGS
  const inherited = encoded === undefined
