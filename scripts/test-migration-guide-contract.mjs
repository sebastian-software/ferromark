...
function validateDocument({ guide, readme, cargoToml, nodePackage, nodeWorkspace, changelog }) {
  assertContains(
    ...
    "must name its supported upgrade range",
  );
  ...
  assertContains(
    ...
    "must explain the Options Copy removal",
  );
  assertContains(
    guide,
    "`#[non_exhaustive]`",
    "must explain the fenced-code forward-compatibility contract",
  );
  ...
  assertContains(
    guide,
    `Node.js ${nodeVersion} or newer`,
    "must state npm package Node requirement",
  );
