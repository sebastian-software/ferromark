...
p.add_argument('--filters',nargs='+',help='Case ID substrings to time; exact HTML checks always cover the full catalog')
...
filters=a.filters or ['lifecycle/','syntax/heading_ids/medium','syntax/allow_link_refs/medium','syntax/footnotes/medium','syntax/tables/medium','core/emphasis/medium','core/links/medium','core/inline_code/medium']
rows=[]
...
result={'exact_html_pairs':len(html['before']),'catalog_sha256':hashlib.sha256(a.catalog.read_bytes()).hexdigest(),'binary_sha256':{k:hashlib.sha256(v.read_bytes()).hexdigest() for k,v in binaries.items()},'rounds':a.rounds,'window_ms':a.window_ms,'filters':filters,'summary':summary,'samples':rows}
