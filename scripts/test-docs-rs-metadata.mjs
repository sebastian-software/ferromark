...
function docsRsNoDefaultFeatures(manifest, document) {
  const match = docsRsTable(manifest, document).match(/^no-default-features\s*=\s*(true|false)\s*$/m)
  if (!match) {
...
function assertDocsRsFeatures(metadata, manifest, document = readFileSync(manifest, 'utf8')) {
  ...
  const expected = ['mdx']
  if (features.length !== expected.length || features.some((name, index) => name !== expected[index])) {
    failContract(
