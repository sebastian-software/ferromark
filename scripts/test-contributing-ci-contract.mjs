...
function markdownSection(document, heading) {
  ...
  return following === -1 ? document.slice(contentStart) : document.slice(contentStart, contentStart + following)
}
...
function validate(root) {
  ...
  if (
    mdxEntries.length !== 1 ||
    !deepEqual(mdxEntries[0], expectedMdxEntry)
  ) {
...
const rustVersion = readRepositoryFile('Cargo.toml').match(/^rust-version\s*=\s*"([^"]+)"\s*$/m)?.[1]
...
describe('CONTRIBUTING CI contract', () => {
  ...
  for (const [label, [original, replacement]] of Object.entries({
    ...
    'required byte-search test': ['cargo test -p ferro-byte-search --locked', 'cargo test -p ferro-byte-search'],
    'required byte-search clippy': ['cargo clippy -p ferro-byte-search --all-targets --locked -- -D warnings', 'cargo clippy -p ferro-byte-search'],
  ...
  it('rejects a stale MSRV statement', () => {
    withFixture(fixtureRoot => {
      ...
      assert.ok(document.includes(requiredMsrv), `test fixture is missing ${JSON.stringify(requiredMsrv)}`)
      writeFileSync(
        file,
        document.replace(
          requiredMsrv,
  ...
  it('rejects a missing isolated mdx matrix entry', () => {
    withFixture(fixtureRoot => {
      ...
      assert.ok(document.includes(mdxEntry), 'test fixture is missing the isolated mdx matrix entry')
