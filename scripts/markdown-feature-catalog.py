...
units = {
    'allow_html': '<div>Raw <em>HTML</em> content.</div>\n\n',
    ...
    'autolink_literals': 'See https://example.org/guide or www.example.org or person@example.org.\n\n',
...
cases=[]
def variant(label,input,options): return {'label':label,'input':input,'options':options}
def add(group,feature,size,variants): cases.append({'id':f'{group}/{feature}/{size}','group':group,'feature':feature,'size':size,'variants':variants})
def configs(feature):
    before={'preset':'commonmark'};after={'preset':'commonmark'}
    if feature in ['merged_table_cells','table_column_widths']: before['tables']=after['tables']=True
    if feature=='disallowed_raw_html': before['trusted']=after['trusted']=True
...
for feature,unit in units.items():
    ...
    for size,text in [('tiny','A short sentence.'),('1k',plain*16),('16k',plain*256)]:
        add('activation',feature,size,[variant('off',text,before),variant('on',text,after)])
    for size,repeats in [('small',1),('medium',16)]:
        text=''.join(unit.replace('{i}',str(i)) for i in range(repeats))
        if feature=='front_matter': text=unit+plain*(repeats-1)
        add('syntax',feature,size,[variant('off',text,before),variant('on',text,after)])
core={
 'plain':plain,'headings':'# Heading\n\nOrdinary words.\n\n',
...
for feature,unit in core.items():
    for size,n in [('small',1),('medium',32)]:
        text=unit*n
        add('core',feature,size,[variant('plain-control','a'*len(text.encode()),{'preset':'commonmark'}),variant('syntax',text,{'preset':'commonmark'})])
for size,text in [('empty',''),('tiny','A short sentence.'),('plain-1k',plain*16),('light-1k',light*12),('plain-16k',plain*256),('readme',pathlib.Path('README.md').read_text())]:
    add('lifecycle','presets',size,[variant(p,text,{'preset':p}) for p in ['commonmark','minimal','gfm','default']])
path=pathlib.Path(sys.argv[1]);path.parent.mkdir(parents=True,exist_ok=True);path.write_text(json.dumps(cases,indent=2)+'\n')
print(f'{len(cases)} cases, {sum(len(c["variants"]) for c in cases)} input/configuration pairs')
